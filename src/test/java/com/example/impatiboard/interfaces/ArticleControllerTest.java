...
import static com.example.impatiboard.fixture.CustomerFixture.*;
...
class ArticleControllerTest {
	...
	void searchArticlesTest() throws Exception {
		// when
		final ArticleSearchRequest request = createSearchRequest();
		final Pageable pageable = PageRequest.of(0, 10);
		final Page<Article> articleResponses = stubArticleResponseWithPage(3, pageable);
		...
		mockMvc.perform(post("/api/v1/articles")
			...
			.andDo(document(
				...
				responseFields(
					...
					fieldWithPath("data.content[].clientType").description(
						"SERVICE_HUB,CUSTOMER_SERVER,HEALTH_CHECKER"),
	...
	void searchArticleTestWithAuthenticationCustomer() throws Exception {
		final String token = "mockToken";
		final Article article = createArticle();
		final AuthenticationCustomer authenticationCustomer = createCustomer(article.getCustomerId());
	...
	void searchArticleTestWithAnonymousCustomer() throws Exception {
		final Article article = createArticle();
		given(articleFinder.search(article.getId())).willReturn(article);
	...
	void postArticlesTest() throws Exception {
		final ArticleRegistrationRequest request = createPostRequest();
		final String token = "mockToken";
		final AuthenticationCustomer authenticationCustomer = createCustomer(1L);
		given(customerFetcher.bringCustomer(token)).willReturn(authenticationCustomer);
		...
		mockMvc.perform(put("/api/v1/articles")
			...
			.andDo(document(
				...
				requestFields(
					...
					fieldWithPath("clientType").type(JsonFieldType.STRING)
						.description("SERVICE_HUB,CUSTOMER_SERVER,HEALTH_CHECKER")),
				responseFields(
	...
	void editArticleTest() throws Exception {
		final Article article = createArticle();
		final ArticleEditRequest request = createEditRequest();
		final String token = "mockToken";
		final AuthenticationCustomer authenticationCustomer = createCustomer(article.getCustomerId());
		given(customerFetcher.bringCustomer(token)).willReturn(authenticationCustomer);
		given(articleEditor.edit(request.toCondition(article.getId()), authenticationCustomer.getId()))
			.willReturn(article);
	...
	List<Article> stubArticleResponse(final int count) {
		List<Article> articleResponses = new ArrayList<>();
		for (int i = 0; i < count; i++) {
			articleResponses.add(createArticle());
		}

		return articleResponses;
	}

	Page<Article> stubArticleResponseWithPage(final int count, final Pageable pageable) {
